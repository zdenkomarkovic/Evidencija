...
async function posaljiPodsetnik(
  ...
  if (!jeKes) {
    ...
    await posaljiEmail({
      ...
      subject,
      html: emailHtml,
      attachment: { content: pdfBuffer, filename: `profaktura-${profakturaBroj}.pdf`, contentType: 'application/pdf' },
    });
    ...
    if (!postojeca) {
      await createFaktura(
        ...
        [{ naziv: 'Hosting usluga - godišnje održavanje', jedinica_mere: 'kom', kolicina: 1, cena: iznos, iznos, redni_broj: 1 }]
      );
    }
  } else {
    const emailHtml = generisiHostingEmailBody(
